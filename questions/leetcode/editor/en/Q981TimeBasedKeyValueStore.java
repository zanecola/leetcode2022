...
// 
//
// Related Topics Hash Table String Binary Search Design 👍 3882 👎 373
...
public class Q981TimeBasedKeyValueStore {
    ...
    class TimeMap {
        ...
        public void set(String key, String value, int timestamp) {
            map.computeIfAbsent(key, (k) -> new TreeMap<>((t1, t2) -> t2 - t1)).put(timestamp, value);
        }

        public String get(String key, int timestamp) {
            var timeValue = map.get(key);
            if (timeValue != null) {
                var entry = timeValue.ceilingEntry(timestamp);
...
class Test {
    public static void main(String[] args) {
        Q981TimeBasedKeyValueStore.TimeMap map = new Q981TimeBasedKeyValueStore().new TimeMap();
        map.set("foo", "bar", 1);
        var a = map.get("foo", 1);
        var b = map.get("foo", 3);
        System.out.println();
